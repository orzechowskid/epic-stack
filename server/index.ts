...
morgan.token("url", (req, res) => {return decodeURIComponent(req.url ?? "")});
app.use(morgan("tiny"));
		...
		contentSecurityPolicy: {
			...
			directives: {
				...
				"script-src": [
					...
					"'self'",
					// @ts-expect-error
					(_, res) => {return `'nonce-${res.locals.cspNonce}'`}
				],
				"script-src-attr": [
					// @ts-expect-error
					(_, res) => {return `'nonce-${res.locals.cspNonce}'`}
...
app.all(
	"*",
	MODE === "development"
		? (...args) => {return getRequestHandler(devBuild)(...args)}
...
const server = app.listen(portToUse, () => {
	...
	const portUsed = desiredPort === portToUse
		? desiredPort
...
closeWithGrace(async () => {
	await new Promise((resolve, reject) => {
		server.close((e) => {return (e ? reject(e) : resolve("ok"))});
	});
});
